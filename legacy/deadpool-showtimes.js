// deadpool-showtimes.js

document.addEventListener('DOMContentLoaded', function() {
  // Selectors
  const dateSlots = document.querySelectorAll('#date-selection .time-slot');
  const timeSlots = document.querySelectorAll('#time-selection-2d .time-slot, #time-selection-imax .time-slot');
    ...
    slots.forEach(slot => {
      slot.addEventListener('click', function() {
        if (type === 'date') {
            ...
            document.querySelectorAll('#time-selection-2d .time-slot, #time-selection-imax .time-slot').forEach(s => s.classList.remove('selected'));
        }
  ...
  selectSeatsBtn.addEventListener('click', function() {
    ...
    const selectedTimeEl = document.querySelector('#time-selection-2d .time-slot.selected, #time-selection-imax .time-slot.selected');
    ...
    const bookingData = {
        eventName: 'Deadpool & Wolverine (TVA Cinema)',
        eventDate: selectedDate,
        showTime: selectedTime,
        tickets: totalTickets,
        subtotal: subtotal
    };
    // Use a unique key for Deadpool's flow data
    localStorage.setItem('deadpoolBookingInfo', JSON.stringify(bookingData));

    // Redirect to the seat selection page
    window.location.href = "deadpool-seats.html";
  });
